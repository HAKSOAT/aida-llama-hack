...
class ModelManager:
    def __init__(self, config: Config):
        self.config = config
        self.model = MllamaForConditionalGeneration.from_pretrained(config.MODEL_NAME, 
...
class LlamaClassification(Inference):
    ...
    def image_to_text(self, input: List[Image.Image]) -> List[str]:
        ...
        combined_image = Image.new('RGB', (sum(image.width for image in input), max(image.height for image in input)))
        x_offset = 0
        for image in input:
        ...
        inputs = self.model.processor(combined_image, prompt, return_tensors="pt").to(self.model.model.device)
...
class LlamaCaption(Inference):
    ...
    def image_to_text(self, input: List[Image.Image]) -> List[str]:
            ...
            conversations.append([
                {
                    "role": "system",
                    "content": [
                        {"type": "text", "text": "You are an intelligent agent who is able to understand satellite images and detect accurately whether there is a natural disaster happening."},
                    ],
                },
                {
                    "role": "user",
        ...
        prompts = [
            self.model.processor.apply_chat_template(conv, add_generation_prompt=True, tokenize=False)
            for conv in conversations
        ]

        # Batch process all inputs
        inputs = self.model.processor(input, prompts, return_tensors="pt", padding=True).to(self.model.model.device)
...
class LlamaAidTagging(Inference):
    def __init__(self, disaster: str, model: ModelManager, aid_resources: Optional[List[str]] = None):
        self.manager = model
        self.disaster = disaster
        if aid_resources:
            self.aid_resources = aid_resources
        else:
            self.aid_resources = ['Food', 'Medication', 'Blankets', 'Water', 'Shelter', 'Rescue transport']
    ...
    def text_to_text(self, input: List[str]) -> List[str]:
        ...
        for commentary in input:
            ...
            system_prompt = "You are an intelligent agent who is able to understand satellite, drone and ground images " \
        ...
        prompts = [
            self.manager.processor.apply_chat_template(conv, add_generation_prompt=True, tokenize=False)
            for conv in conversation
        ]

        inputs = self.manager.processor(None, prompts, return_tensors="pt", padding=True).to(self.manager.model.device)
